  ...
  return (
    <SafeAreaView className='flex-1' edges={['top']}>
      ...
      <ScrollView className='flex-1 px-4' showsVerticalScrollIndicator={false}>
        {/*Banner slider */}
         <View className='mb-6'>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className='w-full h-48 rounded-xl'
        ...
        {/*Pagination Dots */}
         <View className='flex-row justify-center gap-2'>
          {BANNERS.map((_, index)=> (
            <View key={index} className={`h-2 rounded-full ${index === activeBannerIndex ? 'w-4 bg-black' : 'w-2 bg-gray-300'}`} />
          ))}
         </View>
         ...
         <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {categories.map((cat: any)=> (
           <CategoryItem key={cat.id} item={cat} isSelected={false} onPress={() =>  
            router.push({pathname: "/shop", params: {category: cat.id === 'all' ? '' :cat.name}})} />
          ))}
         ...
         <View className='bg-gray-100 p-6 rounded-2xl mb-20 items-center '>
          ...
          <Text className='text=secondary text-center mb-4'> Subscribe to 
            our newsletter and get 10% off on your first purchase.</Text>
