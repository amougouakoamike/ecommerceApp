...
const fetchAddress =  () => {
  ...
  if(addrList.length > 0) {
    ...
    const def = addrList.find((address) => address.isDefault) || addrList[0];
...
if(pageLoading){
   ...
   <SafeAreaView className='flex-1 bg-surface' edges={["top"]}>
    ...
    {selectedAddress ? (
        <View className='bg-white p-4 rounded-xl mb-6 shadow-sm'>
            ...
            <Text className='text-secondary'>{selectedAddress.country}</Text>
        </View>
    ): (
        <TouchableOpacity onPress={()=> router.push('/addresses')}
  ...
  <Text className='text-lg font-bold text-primary mb-4'>
   ...
   className={`bg-white p-4 rounded-xl mb-4 shadow-sm flex-row items-center border-2
    ...
    {paymentMethod === 'cash' && <Ionicons name='checkmark-circle' 
   ...
   className={`bg-white p-4 rounded-xl mb-4 shadow-sm flex-row items-center border-2
    ...
    <Ionicons name='cart-outline' size={24} color={COLORS.primary} className='mr-3'/>
    <View className='ml-3 flex-1'>
        <Text className='text-base font-bold '>Pay with Card</Text>
        <Text className='text-secondary text-xs mt-1'>Credit or Debit Card</Text>
    </View>
    {paymentMethod === 'stripe' && <Ionicons name='checkmark-circle' 
    size={24} color={COLORS.primary}/>}
   </TouchableOpacity>
    </ScrollView>
      ...
      <View className='p-4 bg-white shadow-lg border-t border-gray-100'>
        <Text text-lg font-bold text-primary mb-4>Order Summary</Text>
      ...
      {/* Total Amount */}
        ...
        className={`p-4 mb-4 rounded-xl items-center ${loading ? 'bg-gray-400' : 'bg-black'}`}>
         {loading ? <ActivityIndicator color='white'/> : <Text className='text-white 
         font-bold text-lg'>Place Order</Text>}
        </TouchableOpacity>
